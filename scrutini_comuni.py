...
uri = "/siel/PX/scrutiniFI/DE/20250608/TE/09/RE/{:02d}/PR/{:03d}/CM/{:04d}"
url = dominio + uri
...
base_dir = os.getcwd()
res_dir_name = "results/Scrutini comuni"
    ...
    for info_provincia in (sbar:=tqdm(codici_province[regione], "Provincia", position=1, leave=False)):
        ...
        for info_comune in (cbar:=tqdm(codici_comuni[nome_provincia], "Comune", position=2, leave=False)): 
            ...
            response = requests.get(
                url.format(numero_regione, numero_provincia, numero_comune), headers=headers
            )
